...
void free_layer(layer l)
{
    ...
    if (gpu_index >= 0) {
        ...
        if (l.gate_gpu.ptr) opencl_free(l.gate_gpu);
        if (l.gate_delta_gpu.ptr) opencl_free(l.gate_delta_gpu);
        if (l.save_gpu.ptr) opencl_free(l.save_gpu);
        if (l.save_delta_gpu.ptr) opencl_free(l.save_delta_gpu);
